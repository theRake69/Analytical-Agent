...
def show_home():
    ...
    st.title("Welcome to the Precima Dashboard")

    # Check if session state vartiables are present, if not, initialize them
    ...
    execute_query = st.button("Execute", key="execute")

    if execute_query:
        st.session_state.execute_query = True
        # Keep user query value in session state
        st.session_state.user_query = user_query

    if st.session_state.execute_query and user_query:
    ...
    elif not st.session_state.execute_query:
        st.warning("Please enter a query and click **Execute**")
